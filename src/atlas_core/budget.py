...
class DailyBudgetGuard:
    ...
    def charge(self, cost_usd: float) -> float:
        """Record a run's actual spend; returns the day's new total. Unlike check(), a
        backend error here is swallowed: the money is already spent, and failing the
        finished answer would not unspend it. The next check() still fails closed."""
        try:
            return self._add(cost_usd)
        except Exception as err:
            log.error("budget counter unavailable, $%.4f spend not recorded: %s", cost_usd, err)
            return 0.0
